...
# and for testing.
# When updating the usage string here, also update it in bin/xatag
...
General Options:
  -n --complement      Perform the specified command on the complement of the
                         set of TAG(s) that is specified.  The -n stands for
                         "not".  Can be used on -d, -l, and -c/-C.
     --config-dir=DIR  Use CONFIG-DIR as the xatag configuration, instead of
                         checking in the default path or environment variable.
     --no-index        Do not attempt to update the Recoll index for altered
                         files.
  -q --quiet           Avoid writing to stdout.
  -T --terse           Only print values for tag keys that have been altered.
                         Also, don't print the names of files unless tags will
                         be printed as well.
  -w --no-warn         Do not warn when adding or setting tag values that are
                         not in the known_tags config file.
  -W --warn-once       Print a warning when adding or setting tag values that
                         are not in the known_tags config file, but then add
                         them to the file to prevent future messages.
  -v --version         Print version and exit.
  -h --help            You managed to find this just fine already.

Printing Options:
  -k --key-val-pairs           Print key:val style tag separately, instead of
                                 printing all values with the same key
                                 together.  Probably easier to grep.
                                 Compatible with --one-line.
     --max-padding=INT         Do not pad filenames and tag keys to a length
                                 greater than MAX characters.  A value of 0 is
                                 useful to determine unique tags in a set of
                                 files.  Unlimited by default.
     --min-padding=INT         Pad tag keys to be a length of MIN characters
                                 up.  Useful to try to make tag keys line up
                                 when listing multiple files.  Minimum padding
                                 for filenames and tag values within a single
                                 file is handled automatically.
     --no-print-filename       Do not print the filename, only tag keys and
                                 values.  Useful to determine unique tags in a
                                 set of files.
  -o --one-line                Print all tags on one line.  Possibly easier to
                                 grep.  Compatible with --key-val-pairs.
  -F STR --file-separator=STR  Set character(s) used to separate the file name
                                 from the tags for that file. [default: :] (a
                                 colon)
  -K STR --key-separator=STR   Set character(s) used to separate the tag key
                                 from the tag value.  This only affects
                                 printing, not parsing tags passed as
                                 arguments.  [default: :] (a colon)
  -V STR --val-separator=STR   Set character(s) used to separate tag values.
                                 This only affects printing, not parsing tags
                                 passed as arguments.  [default:  ] (a space)

...
@pytest.fixture
def tag_dict1():
    return {DEFAULT_TAG_KEY: ['some', 'simple', 'tags'],
            'scope': ['home', 'work'],
            'first': ['one', 'tag'],
            'third': ['a', 'b']
            }
...
@pytest.fixture
def tag_dict2():
    return {DEFAULT_TAG_KEY: ['some', 'other', 'tags'],
...
def tag_dict_with_empty_vals():
    return {DEFAULT_TAG_KEY: ['some', 'simple', 'other'],
            'scope': [''],
            'second': ['another'],
            'third': ['a', 'b']
            }
...
def test_merge_tags(tag_dict1, tag_dict2):
    ...
    assert set(m[DEFAULT_TAG_KEY]) == set(['some', 'other', 'simple', 'tags'])
...
def test_subtract_tags(tag_dict1, tag_dict_with_empty_vals):
    ...
    assert set(s[DEFAULT_TAG_KEY]) == set(['tags'])
...
def test_select_tags(tag_dict1, tag_dict_with_empty_vals):
    ...
    assert set(s[DEFAULT_TAG_KEY]) == set(['some', 'simple'])
